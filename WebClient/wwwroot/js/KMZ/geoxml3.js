...
geoXML3.parser = function (options) {
	...
	function fetchDoc() {
		if (index >= listUrls.length) {
			return;
		}
		var thisDoc, j;
		var baseUrl = listUrls[index].split('?')[0];
		thisDoc = new Object();
		thisDoc.baseUrl = baseUrl;
		internals.docSet.push(thisDoc);
		thisDoc.url = listUrls[index];
		thisDoc.internals = internals;
		var url = thisDoc.url;
	...
	var render = function (responseXML, doc) {
		...
		if (!responseXML || responseXML == "failed parse") {
			...
			for (pm = 0; pm < placemarkNodes.length; pm++) {
				...
				// Chi hien thi placeMark co description bat dau la ma cua danh sach truyen vao
				var listCodeShow = options.listCodeShow ? options.listCodeShow : [];
				var chkCode = listCodeShow.find(el => (geoXML3.nodeValue(node.getElementsByTagName('description')[0]) || "").startsWith(el));
				if (chkCode === undefined && listCodeShow.length > 0) {
					continue;
				}
		...
		if (doc.markers.length == 0 && doc.groundoverlays.length == 0 && doc.gpolylines.length == 0 && doc.gpolygons.length == 0) {
			doc.bounds = doc.bounds || new google.maps.LatLngBounds();
			doc.bounds.extend(
				new google.maps.LatLng(16.058030689677206, 108.02723637777794)
			);
			doc.bounds.extend(
				new google.maps.LatLng(16.1144402026136, 108.09452763754356)
			);
		}
		...
		if (!parserOptions.suppressInfoWindows) {
			...
			google.maps.event.addListener(p, 'click', function (e) {
				if (options.clickPolyGon) {
					if (e && e.latLng) {
						options.clickPolyGon(e.latLng, placemark.name);
					} else {
						options.clickPolyGon(p.bounds.getCenter(), placemark.name);
					}
				} else {
					p.infoWindow.close();
			...
			google.maps.event.addListener(p, 'mouseover', function (e) {
				if (options.mouseOverPolygon) {
					if (e && e.latLng) {
						options.mouseOverPolygon(e.latLng, placemark.name);
					} else {
						options.mouseOverPolygon(p.bounds.getCenter(), placemark.name);
					}
				}
			});
		}
